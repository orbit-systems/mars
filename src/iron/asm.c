...
AsmModule* asm_new_module(FeArchInfo* target) {
...
FeAsmInst* asm_add_inst(AsmBlock* b, FeAsmInst* inst) {
    da_append(b, inst);
    return inst;
}

FeAsmInst* asm_new_inst(AsmModule* m, u32 template) {
    FeAsmInst* i = arena_alloc(&m->alloca, sizeof(FeAsmInst), alignof(FeAsmInst));
    *i = (FeAsmInst){0};
...
FeVReg* asm_new_vreg(AsmModule* m, AsmFunction* f, u32 regclass) {
    FeVReg* r = arena_alloc(&m->alloca, sizeof(*r), alignof(*r));
    
    r->required_regclass = regclass;
    r->real = FE_PHYS_UNASSIGNED;
