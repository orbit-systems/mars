...
void debug_asm_printer(FeModule* am) {
    ...
    foreach_non_da(AsmFunction* curr_func, am->functions, am->functions_len, [count]) {
        ...
        foreach_non_da(AsmBlock* curr_block, curr_func->blocks, curr_func->num_blocks, [count]) {
            ...
            foreach(FeAsmInst *curr_inst, *curr_block) {
...
void print_asm_inst(FeModule* m, FeAsmInst* inst, bool debug_mode);

PtrMap vreg2num = {0};
uintptr_t vreg_count = 0;

// debug mode has some extra stuff
void asm_printer(FeModule* m, bool debug_mode) {
...
void print_asm_inst(AsmModule* m, FeAsmInst* inst, bool debug_mode) {

    FeArchInstInfo* info = inst->template;
    ...
    for (size_t i = 0; i < fmt.len; i++) {
        ...
        if (c == '{') {
            ...
            if (strncmp(&fmt.raw[i], "in", 2) == 0) {
                ...
                FeVReg* in = inst->ins[in_index];
                
                if (in->real == FE_PHYS_UNASSIGNED) {
                    ...
                    FeArchRegisterInfo* real = &m->target->regclasses[in->required_regclass].regs[in->real];
                ...
                FeVReg* out = inst->outs[out_index];

                if (out->real == FE_PHYS_UNASSIGNED) {
                    // print a virtual register
                    ...
                    FeArchRegisterInfo* real = &m->target->regclasses[out->required_regclass].regs[out->real];
