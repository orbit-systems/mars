...
static FeArchInfo* target;

static bool is_defined_by(FeAsmInst* inst, FeVReg* vreg) {
...
static bool is_used_by(FeAsmInst* inst, FeVReg* vreg) {
...
static void alloc(AsmFunction* f, PtrMap* vreg2liverange) {
    foreach(FeVReg* v, f->vregs) {
        if (v->real != FE_PHYS_UNASSIGNED) continue;

        LiveRange* range = ptrmap_get(vreg2liverange, v);
        assert(range != PTRMAP_NOT_FOUND);
        
        FeArchRegisterClass* regclass = &target->regclasses[v->required_regclass];
