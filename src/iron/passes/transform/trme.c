...
static FeInstStore* last_local_store_to_loc(FeBasicBlock* bb, FeInst* location, u64 from) {
    if (location == NULL) return NULL;
    for (i64 i = from-1; i >= 0; i--) {
        if (bb->at[i]->kind == FE_INST_STORE && ((FeInstStore*)bb->at[i])->location == location) {
            return (FeInstStore*) bb->at[i];
        }
    }
    return NULL;
}
    ...
    for (u64 i = 0; i < mod->functions_len; i++) {
        ...
        for (u64 j = 0; j < f->blocks.len; j++) {
            ...
            for (u64 inst = 0; inst < bb->len; inst++) {
                if (bb->at[inst]->kind != FE_INST_LOAD) continue;

                FeInstLoad* ir = (FeInstLoad*) bb->at[inst];  
                FeInstStore* last_store = last_local_store_to_loc(bb, ir->location, inst);
                ...
                ir->base.kind = FE_INST_MOV;
                ((FeInstMov*)ir)->source = last_store->value;
            }
        }
        ...
        for (u64 j = 0; j < f->blocks.len; j++) {
            ...
            for (u64 inst = 0; inst < bb->len; inst++) {
                if (bb->at[inst]->kind != FE_INST_STACKADDR) continue;
                ...
                remove_stack_slot(f, ((FeInstStackAddr*)bb->at[inst])->object);
                bb->at[inst]->kind = FE_INST_ELIMINATED;

                // eliminate the stores as well
                for (u64 s = 0; s < store_elim_list.len; s++) {
                    store_elim_list.at[s]->kind = FE_INST_ELIMINATED;
                }
