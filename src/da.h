...
#define da_init(da_ptr, capacity) do { \
    ...
    if ((da_ptr)->at == NULL) { \
        printf("(%s:%ld) da_init malloc failed for capacity %ld", (__FILE__), (__LINE__), c); \
        exit(1); \
    } \
} while (0)
...
#define da_append(da_ptr, element) do { \
    if ((da_ptr)->len == (da_ptr)->cap) { \
        ...
        if ((da_ptr)->at == NULL) { \
            printf("(%s:%ld) da_append realloc failed for capacity %ld", (__FILE__), (__LINE__), (da_ptr)->cap); \
            exit(1); \
        } \
    } \
...
#define da_shrink(da_ptr) do { \
    ...
    (da_ptr)->at = realloc((da_ptr)->at, sizeof((da_ptr)->at[0]) * (da_ptr)->cap); \
    if ((da_ptr)->at == NULL) { \
        printf("(%s:%ld) da_init realloc failed for capacity %ld", (__FILE__), (__LINE__), (da_ptr)->cap); \
        exit(1); \
    } \
} while (0)
...
#define da_reserve(da_ptr, num_slots) do { \
    ...
    (da_ptr)->at *= realloc((da_ptr)->at, sizeof((da_ptr)->at[0]) * (da_ptr)->cap); \
    if ((da_ptr)->at == NULL) { \
        printf("(%s:%ld) da_reserve realloc failed for capacity %ld", (__FILE__), (__LINE__), (da_ptr)->cap); \
        exit(1); \
    } \
} while (0)
