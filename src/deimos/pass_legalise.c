...
void recurse_legalisation_ast(DAG parent_link, DAG parent, AST node, int depth) {
	switch (node.type) {
		...
		//statements
		case AST_decl_stmt: { //?FIXME: is there a better way of doing this? do we need decl_stmt?
			...
			parent_link.as_decl_stmt->is_volatile = node.as_decl_stmt->is_volatile;
			parent_link.as_decl_stmt->is_uninit   = node.as_decl_stmt->is_uninit;
			break;
		}
		case AST_block_stmt: {
			parent_link = new_dag_node(&dag_alloca, dagtype_block_stmt);
		...
		case AST_return_stmt: {
			parent_link = new_dag_node(&dag_alloca, dagtype_return_stmt);
		...
		case AST_func_literal_expr: {
			parent_link = new_dag_node(&dag_alloca, dagtype_func_literal);
		...
		case AST_binary_op_expr: {
			parent_link = new_dag_node(&dag_alloca, dagtype_binary_op);
