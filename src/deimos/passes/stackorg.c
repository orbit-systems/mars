...
int sort_stackalloc(const void* a, const void* b) {
	IR* ir_a = *(IR**)a;
	IR* ir_b = *(IR**)b;
	if (ir_a->tag == IR_PARAMVAL && ir_b->tag != IR_PARAMVAL) return -1;
	if (ir_a->tag == IR_PARAMVAL && ir_b->tag == IR_PARAMVAL) {
		if (((IR_ParamVal*)ir_a)->param_idx > ((IR_ParamVal*)ir_b)->param_idx) return -1;
		else return 1;
	}
