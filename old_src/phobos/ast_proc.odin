...
get_pos :: proc(node: AST) -> position {
    #partial switch n in node {
    case ^ident_expr:
        return n.tok.pos
    case ^decl_stmt:
        if len(n.rhs) == 0 {
            return merge_pos(get_pos(n.lhs[0]), get_pos(n.types[len(n.types)-1]))
        }
        return merge_pos(get_pos(n.lhs[0]), get_pos(n.rhs[len(n.rhs)-1]))
    case:
        TODO("unhandled case %v", node)
    }
